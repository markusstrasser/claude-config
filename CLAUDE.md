...
**Measure the model, don't cite a paper about it (model-behavior questions only).** For *behavioral* properties of the model you're using NOW — judge bias, calibration, refusal, preference, sycophancy — published papers are structurally a generation behind: a 2026 paper was run on 2024/2025-frontier models over its 12-18mo write/review cycle, and these properties are products of per-release preference-training, NOT scale-independent. So the literature gives *mechanism* (why self-preference/correlated-errors exist) but cannot give the *current rate*. When the question is "is the judge/model in my pipeline biased, and how much," run a direct probe on the live model — it's cheap (minutes) and it's the only frontier-valid evidence. Keep the probe as a re-runnable instrument; re-run when models ship. (Evidence: 2026-06-11 — a 66-call probe of GPT-5.5/Gemini-3.5/Opus-4.8/Fable-5 found position bias *solved* but verbosity bias *persisting and inverting by lab* (OpenAI/Google prefer padded filler 8-9/10, Anthropic prefer concise 9-10/10), effort-robust — directly refuting a Nov-2025 "all models prefer concise" finding. `agent-infra research/2026-06-11-frontier-judge-bias-measured.md`.)
