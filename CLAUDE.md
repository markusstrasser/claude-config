...
1. **Does this already exist? Has this problem actually occurred?** Check vendor GitHub/changelog/SDKs, OSS, and the codebase before building. Search by FUNCTIONALITY (decorators, signatures, stage names), not filenames. For NEW infrastructure: `git log --grep` for incidents it would prevent — no incident history → hypothetical → default to not building. For DEFERRED plans: `git log --oneline -20 -- <paths>` before resuming. **Discovery is YOUR standing job — proactive, planning-time, across the FULL dependency tree (#g 2026-06-21).** Scope research to what a winning solution NEEDS, not the question literally asked: for every capability the plan leans on — optimizer, training recipe, serving stack, dataset, benchmark, the SOTA system to beat — scout the external frontier (OSS, arXiv, leaderboards) AND your own prior memos for the BEST existing tool, not merely whether the feature exists, BEFORE proposing to build or train. Litmus: if a human hands you a canonical resource in your strong domain (a famous optimizer, a reference repo, an existing in-repo memo), you failed discovery — that's your edge, own it. The autonomous portfolio therefore carries a SCOUT front (find-what-exists) beside grind/heretic/dreamer/meta. **For EDGE/MOAT-framed infra, the bar is "does a TOOLED agent already do this?" — run the kill-switch eval BEFORE building, not after (#g 2026-06-30).** When the *justification* for building is an edge / a moat / a capability frontier agents lack, plain "does it exist" is too weak — the real test is whether a frontier agent **with web + filesystem + code tools** already does the job on-demand. Answer it FIRST with the same cheap eval you'd otherwise run as a post-hoc kill-switch: stand up the tooled-agent baseline and race it against the thing you'd build, *before* building. **Substrate is the cautionary arc** — built over months, then killed (2026-06-29) by an S4-S8 kill-switch eval finding no capability edge over a tooled agent (verify / multi-hop / completeness all tied; the private-data loophole closed because the agent just greps the local drive), a verdict the 2026 literature corroborated (verification ledgers commoditized: OSS MCP, OriginTrail DKG, PaperQA2). That eval was ~a day; run before the build, it *prevents* the build. **Bounded to edge/moat framing** — NOT a universal pre-build gate (that way lies paralysis): a build justified by ergonomics, determinism, latency, cost, or a concrete in-hand consumer owes no kill-switch eval. Trigger is the *word* "edge"/"moat"/"agents can't" in the rationale.
...
**Safety:** Analysis subagents must not commit. Default `isolation: "worktree"` for any subagent **mutating shared code files in parallel** — hard isolation beats soft by 7.8pp; soft isolation HURTS on open-ended tasks (CAID, arXiv:2603.21489). The trigger is *parallel mutation of a shared file*, NOT "touched code": an additive-output agent (research memo / analysis writing a NEW file) needs no worktree — it returns the path and the PARENT commits; isolating it buys zero collision-safety and adds a pure merge-back tax (the worktree-cleanup chore).
